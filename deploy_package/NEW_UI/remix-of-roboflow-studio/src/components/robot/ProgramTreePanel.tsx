  ...
  return (
    <div>
      <div
        ...
        {hasChildren ? (
          node.expanded ? <ChevronDown size={10} className="text-muted-foreground shrink-0" /> : <ChevronRight size={10} className="text-muted-foreground shrink-0" />
        ) : <span className="w-3 shrink-0" />}
        <Icon size={13} strokeWidth={1.5} className={node.type === 'waypoint' ? 'text-status-info' : node.type === 'routine' ? 'text-status-ok' : 'text-muted-foreground'} />
